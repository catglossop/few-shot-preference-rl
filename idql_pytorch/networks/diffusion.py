...
def ddpm_sampler(actor_apply_fn, actor_params, T, rng, act_dim, observations, alphas, alpha_hats, betas, sample_temperature, repeat_last_step, clip_sampler, training = False):
    ...
    def fn(input_tuple, time):
        ...
        rng = jnp.random.default_rng(rng)
        z = rng.normal(size=(observations.shape[0], current_x.shape[1]))
        ...
        # Assuming rng.integers returns a new seed for RNG
        new_rng_seed = rng.integers(0, 1e9)

        return (current_x, new_rng_seed), ()

    rng = jnp.random.default_rng(rng)
    # TODO: find a replacement for jax.lax.scan
