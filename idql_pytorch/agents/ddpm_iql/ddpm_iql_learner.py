...
import numpy as jnp
import optax
from agents.agent import TrainState
import numpy as np
from agents.agent import Agent
from data.dataset import DatasetDict
from networks import MLP, Ensemble, StateActionValue, StateValue, DDPM, FourierFeatures, cosine_beta_schedule, ddpm_sampler, MLPResNet, vp_beta_schedule
...
def exponential_loss(diff, beta, clip=jnp.log(6), mode='zero'):
    ...
    # TODO: find equivalent of jax.lax.stop_gradient
...
class DDPMIQLLearner(Agent):
    ...
    # TODO: verify if we can remove struct.field like this --> struct field tells jax to not be traine don these
    # critic_objective: str = struct.field(pytree_node=False)
    # actor_objective: str = struct.field(pytree_node=False)
    # act_dim: int = struct.field(pytree_node=False)
    critic_objective: str
    actor_objective: str
    act_dim: int 
    ...
    def create(
        ...
        actor_key, critic_key, value_key = rng.random(), rng.random(), rng.random()
    ...
    def update_v(agent, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        ...
        # TODO: find replacement for grad (torch.Tensor.grad or torch.autograd.grad)
    ...
    def update_actor(agent, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        rng = agent.rng
        key, rng = rng.random(), rng.random()
        time = np.random.randint(0, agent.T, size=batch['actions'].shape[0])
        noise_sample = np.random.normal(size=(batch['actions'].shape[0], agent.act_dim))
    ...
    def sample_implicit_policy(self, observations: jnp.ndarray):
        ...
        if self.critic_objective == 'expectile':
            ...
            sample_idx = np.random.choice(self.N, p=tau_weights/tau_weights.sum())
            ...
            sample_idx = np.random.choice(self.N, p=weights)
            action = actions[sample_idx]
        else:
