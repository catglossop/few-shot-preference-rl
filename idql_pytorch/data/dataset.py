...
from types_idql import DataType
...
class Dataset(object):
    ...
    def sample_jax(self, batch_size: int, keys: Optional[Iterable[str]] = None):
        if not hasattr(self, "rng"):
            self.rng = np.random.RandomState(self._seed or 42)
            ...
            def _sample_jax(rng):
                indx = np.random.randint(0, len(self), size=batch_size)
                return {k: np.take(v, indx, axis=0) for k, v in jax_dataset_dict.items()}
                # TODO: find a replacement for jax.tree_map
